...
아쉬웠던 점은 단위 테스트와 통합 테스트를 충분히 작성하지 못해 런타임에 예상치 못한 버그를 발견하는 경우가 많았다는 것으로, 다음 프로젝트에서는 테스트 커버리지를 높여 안정성을 확보하고자 합니다.

복잡한 MSA 환경에서 발생한 다양한 기술적 문제들을 팀원들과 함께 해결해나가면서, 단순히 코드를 작성하는 것을 넘어 시스템 전체를 설계하고 안정성을 확보하는 능력을 기를 수 있었습니다.
